...
<script>
import {getProductAttrCateList} from "@/api/productAttrCate";
...
export default {
  ...
  methods: {
    listForProductAttrCate() {
      let listQuery = {pageNum: 1, pageSize: 100};
      getProductAttrCateList(listQuery).then(response => {
        this.productAttrCateList = response.data.data.list;
      });
    },
    resetProductAttr() {
